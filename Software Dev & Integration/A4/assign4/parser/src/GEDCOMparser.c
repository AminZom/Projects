...
char* indToJSON(const Individual* ind)  //Function creates a JSON string that describes an individual
{
    ...
    if(ind != NULL)
    {
		sprintf(str, "{\"givenName\":\"%s\",\"surname\":\"%s\",\"sex\":", ind->givenName, ind->surname);
		ListIterator sexFinder = createIterator(ind->otherFields);
		Field* field;
		while((field = nextElement(&sexFinder)) != NULL)
		{
			if(strcmp(field->tag, "SEX") == 0)
			{
				sprintf(str, "%s\"%s\"}", str, field->value);
				return str;
			}
		}
		sprintf(str, "%s\"N/A\"}", str);
		return str;
	}
    return str;
}
...
char* createGEDCOMjson(char* fileName)
{
	char* JSONstr = malloc(sizeof(char) * 5000);
	GEDCOMobject* obj = NULL;
	GEDCOMerror error = createGEDCOM(fileName, &obj);
	if(error.type != OK)
	{
		strcpy(JSONstr, "{\"Error\":\"Yes\"}");
		return JSONstr;
	}
	sprintf(JSONstr, "{\"Error\":\"No\",\"fileName\":\"%s\",\"source\":", fileName);
	if(obj->header->source != NULL)
		sprintf(JSONstr, "%s\"%s\",", JSONstr, obj->header->source);
	else
		sprintf(JSONstr, "%s\"\",", JSONstr);
	sprintf(JSONstr, "%s\"gedcVersion\":\"%.1f\",\"encoding\":", JSONstr, obj->header->gedcVersion);
	if(obj->header->encoding == ASCII)
		strcat(JSONstr, "\"ASCII\",\"submitterName\":");
	else if(obj->header->encoding == ANSEL)
		strcat(JSONstr, "\"ANSEL\",\"submitterName\":");
	else if(obj->header->encoding == UTF8)
		strcat(JSONstr, "\"UTF8\",\"submitterName\":");
	else if(obj->header->encoding == UNICODE)
		strcat(JSONstr, "\"UNICODE\",\"submitterName\":");
	sprintf(JSONstr, "%s\"%s\",\"submitterAddress\":", JSONstr, obj->submitter->submitterName);
	if(obj->submitter->address == NULL || strcmp(obj->submitter->address, "") == 0)
		strcat(JSONstr, "\"\",");
	else
		sprintf(JSONstr, "%s\"%s\",", JSONstr, obj->submitter->address);
	int indivsLength = getLength(obj->individuals);
	int famsLength = getLength(obj->families);
	if(indivsLength < 0)
		indivsLength = 0;
	if(famsLength < 0)
		famsLength = 0;
	sprintf(JSONstr, "%s\"indivsNum\":\"%d\",\"famsNum\":\"%d\",\"indivs\":", JSONstr, indivsLength, famsLength);
	if(indivsLength != 0)
		sprintf(JSONstr, "%s%s}", JSONstr, iListToJSON(obj->individuals));
	else
		sprintf(JSONstr, "%s\"none\"}", JSONstr);
	deleteGEDCOM(obj);
	return JSONstr;
}

void createSimpleGEDCOM(char* JSONstr, char* fileName)
{
	GEDCOMobject* obj = JSONtoGEDCOM(JSONstr);
