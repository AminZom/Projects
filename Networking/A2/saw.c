/* ******************************************************************
RDT network emulator by Jim Kurose, edited by Xiaodong Lin
used to implement a Reliable Transport Protocol, such as the Stop-and-Wait (SAW)
or Go-Back-N (GBN) protocols
...
int A_acknum;
int checkSend;
struct pkt prevSentPkt;

int B_seqnum;
// End of your variables and constants
...
void A_init(void) {
    A_seqnum = -1;
    A_acknum = 0;
    checkSend = 0;	//Flag used to check if there is a message in transmission
}
...
void A_send(struct msg message) {
	...
	{
		struct pkt sendpkt;
		A_seqnum++;		//Incrementing the sequence number once before every packet is sent
		sendpkt.seqnum = A_seqnum;
		sendpkt.acknum = A_acknum;
		sendpkt.checksum = 0;
		strcpy(sendpkt.payload, message.data);
		for(int i = 0; i < strlen(sendpkt.payload); i++)	//Calculating the checksum of the message
			sendpkt.checksum += sendpkt.payload[i];
			
		checkSend = 1;	//Setting the check flag once the packet is in transmission to B, so any new incoming packets are dropped
		prevSentPkt = sendpkt;	//Saving the packet sent
		to_network_layer(0, sendpkt);
		starttimer(0, 10);
	}
}
...
void A_recv(struct pkt packet) {
	stoptimer(0);	//Stopping timer once the ACK packet was received
	int AcheckSum = 0;
    for(int i = 0; i < strlen(packet.payload); i++)	//Calculating the checksum of the payload
		AcheckSum += packet.payload[i];
    if(AcheckSum == packet.checksum && packet.acknum == packet.seqnum + 1)	//Checking to see if whether or not the packet is corrupt
    {
		A_acknum = packet.seqnum + 1;	//Setting the correct acknum if the packet is not corrupt
		checkSend = 0;	//Allowing the A_send() function to receive new messages
	}
	else
	{
		to_network_layer(0, prevSentPkt);	//Re-sending the previously sent packet (which was corrupted before)
		starttimer(0, 10);
	}
}
...
void A_timeout(void) {
    to_network_layer(0, prevSentPkt);	//Re-sending the previously sent packet (which was timed out/lost)
    starttimer(0, 10);
}
...
void B_recv(struct pkt packet) {
	int BcheckSum = 0;
    for(int i = 0; i < strlen(packet.payload); i++)		//Calculating the checksum of the payload
		BcheckSum += packet.payload[i];
	if(BcheckSum == packet.checksum && packet.acknum == packet.seqnum)	//Checking to see if whether or not the packet is corrupt
	{
		packet.acknum = packet.seqnum + 1;	//Setting the correct acknum within the packet if the packet is not corrupt
		if(B_seqnum != packet.seqnum)	//Checking for duplicate packet sequences
		{
			B_seqnum = packet.seqnum;
			to_application_layer(1, packet.payload);	//Sending data to app layer if it is not a duplicate packet
		}
	}
	
	to_network_layer(1, packet);	//Sending packet back to A
}
